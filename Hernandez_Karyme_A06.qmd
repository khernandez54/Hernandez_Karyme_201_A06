...
[https://github.com/khernandez54/Hernandez_Karyme_201_A06)](https://github.com/khernandez54/Hernandez_Karyme_201_A06)
...
```{r}
player_height <- list(MJ = 78, LJ = 81, KB = 78, KA = 86, SO = 85)
...
```{r}
points_rounded <- sapply(points_per_game, function(x){round(x, 2)})
...
apply(mat_rand, MARGIN = 1:2, round_function)
```
...
```{r}
original_max_temps <- max_temps
max_temps$state <- NULL
max_temps$year <- NULL

apply(max_temps, 2, mean)
```
...
```{r}
CO_max_temps <- original_max_temps[original_max_temps$state == "CO", ]
OH_max_temps <- original_max_temps[original_max_temps$state == "OH", ]
VA_max_temps <- original_max_temps[original_max_temps$state == "VA", ]
```
...
```{r}
med_temps <- numeric(nrow(CO_max_temps))

for(i in 1:nrow(CO_max_temps)){
  yearly_temp <- as.numeric(CO_max_temps[i, -(1:2)]) # exclude state and year cols
  med_temps[i] <- median(yearly_temp) 
}

max_med_temp <- max(med_temps)
min_med_temp <- min(med_temps)

max_year <- CO_max_temps$year[which.max(med_temps)]
min_year <- CO_max_temps$year[which.min(med_temps)]

print(paste0("The maximum median annual temperature of ", max_med_temp, " occurred in ", max_year, " and the minimum median annual temperature of ", min_med_temp, " occurred in ", min_year,"."))
```
...
```{r}
digits_sum_num <- function(x){
    if(x < 10){
        return(x)
    }else{
        return((x %% 10) + digits_sum_num(x %/% 10))
    }
}

digits_sum_num(935068)
```
